...
#         x1, y1, x2, y2 = bbox
#         x1 = max(0, min(x1, orig_width_cpu - 1))
#         y1 = max(0, min(y1, orig_height_cpu - 1))
#         x2 = max(0, min(x2, orig_width_cpu - 1))
#         y2 = max(0, min(y2, orig_height_cpu - 1))
...
def setup_model(MODEL_NAME):
    if not torch.cuda.is_available() or torch.cuda.device_count() == 0:
        ...
        raise NotImplementedError("This script is currently not supported on CPU. Please use a GPU for inference.")
        # device_map_arg = "cpu"
        ...
        device_map_arg = "auto"
        # max_memory_arg = {x: "75GiB" for x in range(torch.cuda.device_count())}
...
def grounding_pipeline_batched(
    ...
    return all_json_responses, None
